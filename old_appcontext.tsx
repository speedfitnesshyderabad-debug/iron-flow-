...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      if (typeof Notification !== 'undefined' && Notification.permission === 'default') {
        ...
        Notification.requestPermission().then(permission => {
          if (permission === 'granted') {
            new Notification(`≡ƒöö ${bName} Notifications Enabled`, {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (authUser) {
        // ≡ƒöì Smart Identity Sync: Check by BOTH ID and Email
        ...
        if (existingProfile) {
          // ≡ƒöä Handle UID Shift (e.g. Google Login -> Email/Password Login with same email)
          if (existingProfile.id !== authUser.id) {
            console.log('≡ƒöä Identity mismatch detected, migrating profile ID to new Auth UID...');
            ...
            if (!updateError && updatedProfile) {
              console.log('Γ£à Identity migrated successfully');
              setCurrentUser(updatedProfile);
            } else {
              console.error('Γ¥î Identity migration failed:', updateError);
          ...
          // ≡ƒ¢í∩╕Å Secure Profile Reconstruction (If DB record is entirely missing)
      ...
      if (subError) console.warn('ΓÜá∩╕Å Subscription join failed:', subError);
        ...
        if (payload.eventType === 'INSERT') {
          ...
          if (currentUserRef.current?.id === newComm.user_id) {
            ...
            if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
              new Notification(`≡ƒöö New Notification`, {
                body: newComm.body,
                icon: '/favicon.ico'
              });
            }
  ...
  useEffect(() => {
    if (currentUser?.id) {
      console.log('≡ƒöä User logged in/changed, syncing data...');
      fetchData();
    }
  ...
  const addUser = async (u: User) => {
    ...
    if (!error) {
      ...
      console.error('Γ¥î addUser error:', error);
  ...
  const deleteUser = async (id: string) => {
    ...
    try {
      ...
      for (const table of cleanupTables) {
        ...
        if (error) {
          console.warn(`ΓÜá∩╕Å Cleanup failed for table ${table}:`, error.message);
        }
      ...
      if (!deleteResp.ok) {
        ...
        console.warn('ΓÜá∩╕Å Auth deletion failed (non-critical):', errData);
      }
      ...
      console.error('Γ¥î Deletion failed:', e);
  ...
  const sellInventoryItem = async (itemId: string, memberId: string, quantity: number, paymentMethod: 'CASH' | 'POS' | 'CARD' | 'ONLINE', transactionCode?: string, razorpayPaymentId?: string) => {
    ...
    // ≡ƒò╡∩╕Å Fetch member name for UI join if not in global state
    ...
    const newSale: Sale = {
      ...
      memberId,
      itemId,
      staffId: currentUser?.id || null, // Γ¥î Fix FK violation: No 'pos' placeholder
      branchId,
      paymentMethod,
  ...
  const sendNotification = async (comm: Omit<Communication, 'id' | 'timestamp' | 'status'>, emailOverride?: string) => {
    ...
    if (!error) {
      ...
      // ≡ƒôº Real Email Delivery via SendGrid (Supabase Edge Function)
      ...
      if (emailTo && emailTo.includes('@')) {
        ...
        const categorySubjects: Record<string, string> = {
          WELCOME: `≡ƒæï Welcome to ${bName}!`,
          PAYMENT: `≡ƒÆ│ Payment Confirmation ΓÇô ${bName}`,
          REMINDER: `ΓÅ░ Reminder from ${bName}`,
          ANNOUNCEMENT: `≡ƒôó Announcement from ${bName}`,
        };
        const emailSubject = newComm.subject || categorySubjects[newComm.category] || `≡ƒöö ${bName} Notification`;
        const emailFrom = branch?.emailFromAddress || 'admin@speedfitness.org';
        const emailFromName = branch?.name || 'IronFlow Gym';

        // Γ£à Supabase Edge Function ΓÇö no Vercel needed
        const supabaseUrl = import.meta.env.VITE_SUPABASE_URL?.trim();
        ...
        fetch(emailEndpoint, {
          ...
          body: JSON.stringify({
            ...
            branchName: bName,
            apiKey: branch?.emailApiKey, // From Branch Settings ΓåÆ Email Infrastructure
          }),
        }).catch(e => console.warn('≡ƒôº Email send failed (non-critical):', e));
      }

      // ≡ƒô▒ Real SMS Delivery via Twilio (Supabase Edge Function)
      ...
      if (isPhoneNumber && hasTwilioKey) {
        ...
        fetch(smsEndpoint, {
          ...
          .then(async r => {
            ...
            if (!r.ok) console.warn('≡ƒô▒ SMS send failed:', resp);
            else console.log('≡ƒô▒ SMS sent:', resp.messageSid);
          })
          .catch(e => console.warn('≡ƒô▒ SMS send failed (non-critical):', e));
      } else if (isPhoneNumber && !hasTwilioKey) {
        console.warn('≡ƒô▒ SMS skipped: No Twilio API key in Branch Settings ΓåÆ SMS Gateway.');
      }

      // ≡ƒöö Browser Push Notification
      ...
      if (
        ...
        const categoryTitles: Record<string, string> = {
          WELCOME: `≡ƒæï Welcome to ${bName}!`,
          PAYMENT: '≡ƒÆ│ Payment Confirmed',
          REMINDER: 'ΓÅ░ Reminder',
          ANNOUNCEMENT: '≡ƒôó New Announcement',
        };
        const title = categoryTitles[newComm.category] || `≡ƒöö ${bName} Notification`;
        try {
  ...
  const enrollMember = async (userData: Partial<User>, planId?: string, trainerId?: string, password?: string, discount: number = 0, paymentMethod: 'CASH' | 'CARD' | 'ONLINE' | 'POS' = 'ONLINE', startDate?: string, staffId?: string, referralCode?: string, pauseAllowance: number = 0) => {
    ...
    try {
      ...
      // ΓÖ╗∩╕Å Handle "User already registered" ΓÇö stale Auth user from a previous deletion
      // Auto-clean the stale Auth record and retry signup
      let finalAuthData = authData;
      if (authError?.message?.toLowerCase().includes('already registered') || authError?.message?.toLowerCase().includes('already been registered')) {
        console.warn('ΓÜá∩╕Å Stale Auth user detected, cleaning up and retrying...');
        const supabaseUrlClean = import.meta.env.VITE_SUPABASE_URL?.trim();
      ...
      const newUser: User = {
        ...
        email: userData.email || '',
        // password: password, // Γ¥î SECURITY: Do NOT store the password!
        role: UserRole.MEMBER,
        branchId,
      ...
      if (dbError) {
        console.error('Γ¥î Database Profile Creation Failed:', dbError);
      ...
      if (plan) {
        ...
        const newSub: Subscription = {
          ...
          // Keep for local state only ΓÇö not in DB schema
          saleId: saleId,
        ...
        // Strip virtual fields (member, saleId) before DB insert ΓÇö they don't exist in the schema
        ...
        if (subError) {
          console.error('Γ¥î Subscription Creation Failed:', subError);
        ...
        if (saleError) {
          console.error('Γ¥î Sale Creation Failed:', saleError);
      ...
      await sendNotification({
        ...
        category: 'WELCOME',
        branchId: branchId
      }, userData.email); // Γ£à emailOverride bypasses stale users state
  ...
  const purchaseSubscription = async (userId: string, planId: string, paymentMethod: 'CASH' | 'CARD' | 'ONLINE' | 'POS', trainerId?: string, referralCode?: string, pauseAllowance: number = 0, discount: number = 0, customStartDate?: string) => {
    ...
    // ≡ƒò╡∩╕Å Member Retrieval: Members are excluded from global state. 
    // Fetch directly from DB if not found locally.
    if (!user) {
      console.log('≡ƒöä Member not in local state, fetching from DB...');
      ...
      if (userError || !fetchedUser) {
        console.error('Γ¥î Member lookup failed:', userError);
        setGlobalLoading(false);
    ...
    // ≡ƒ¢í∩╕Å Robustness: Fallback to current staff's branch if member has none assigned yet
    const branchId = user.branchId || currentUser?.branchId || branches[0]?.id || null;

    const saleId = crypto.randomUUID();

    // Build DB-safe objects ΓÇö strip virtual join fields not in schema
    const newSub: Subscription = {
      id: crypto.randomUUID(),
      memberId: userId,
      planId: planId,
    ...
    try {
      ...
      console.error('Γ¥î Transaction failed:', e);
  ...
  const validateCoupon = async (code: string, planId: string) => {
    ...
    if (coupon.minPurchase && plan.price < coupon.minPurchase) {
      return { valid: false, discount: 0, message: `Minimum purchase of Γé╣${coupon.minPurchase} required` };
    }
